...
Every v5 change is compile-visible, so the compiler finds each affected site:

- The Go import path gains `/v5`: `github.com/cplieger/web-terminal-engine/v5/terminal` (and `/v5/vt`). The TS package name is unchanged.
- `WithoutSessionReap` is removed — session reaping is unconditional; work meant to outlive a tab belongs outside the session's process tree (a detached service), not behind an engine knob.
- `WithInputTitle` is removed — a client that wants a typed-first-line label pushes it via `PUT /api/sessions/{id}/title` (`SetSessionTitle`); it ranks below a program's own OSC title in the resolved `title`.
- Same-typed argument lists were replaced by labeled shapes: the manager surface speaks `terminal.SessionID`, `WirePairIncompatibility` takes two `WireEnd` halves, `MountSessionRoutes` takes a `SessionHandlers` struct, `NewContainment` takes a `CgroupRoot`, and `WithKeepUnfocused` takes an explicit bool.
